...
static float GetShake(CameraManager *manager)
{
	return manager->trauma * manager->trauma;
}

static void CameraAddTrauma(CameraManager *manager, float amount)
{
	manager->trauma = Clamp(manager->trauma + amount, 0.0, 1.0);
}

static void TickCameraManager(CameraManager *manager, float delta)
{
	CameraAddTrauma(manager, -TRAUMA_LOSS_PER_SECOND * delta);
}

static Camera2D GetModifiedCamera(CameraManager *manager)
{
